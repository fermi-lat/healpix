...
template<typename T> void saf_constantnoise(Alm<T> &sky,Alm<T> &psf,double cnl){
	...
	for(int l=1;l<=psf.Lmax();l++) {
		...
		xcomplex<T> fact(l/cnl,0);
		//FIXME, b += fact*(psf(l,0).conj()-psf(l-1,0));
		//FIXME, c += fact*(psf(l,0)-psf(l-1,0));
	}
	//FIXME, double del = sqrt((a*c-b.norm()).norm());
	double del(0.);
	arr<double> scale(psf.Lmax()+1,0);
	for(int l=1;l<=psf.Lmax();l++) {
	         //FIXME, scale[l] = (c-b*l*(psf(l,0)-psf(l-1,0)));
  	         scale[l] = 0.;
	}
	sky.Scale(-1/(del*cnl));
	sky.ScaleL(scale);
...
template<typename T> void saf_noise(Alm<T> &sky,Alm<T> &psf,Alm<T> &noise){
	...
	for(int l=1;l<=psf.Lmax();l++) {
		...
		//FIXME, scale[l] = (c-b*l*(psf(l,0)-psf(l-1,0)))*fact;
                scale[l] = 0.;
	}
	sky.Scale(-1/del);
	sky.ScaleL(scale);
...
#endif
{
    //FIXME, Ylmgen generator(lmax,mmax,1e-30);
    arr<double> Ylm;
...
#pragma omp for schedule(dynamic,1)
	...
	//FIXME, generator.get_Ylm(cthr,sthr,m,Ylm,l);
        if (l<=lmax)
          {
